    ...
    fun load(file: String): Phase {
        ...
        for (i in 1..theo.count() - 1) {
            ...
            if (cell.isNotEmpty()) {
                ...
                if (phaseLink == null) throw IllegalStateException("${row[1].trim()} ${row[2].trim()} is not matching phases")
                ...
                if (row[7].trim().isNotBlank()) {
                    ...
                    val author = phase.authors.addReturn(
                            Author(name = PersonName(firstName = row[6].trim(), lastName = row[7].trim(), link = row[8].trim()),
